{{-- resources/views/secretaria/appointments/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Agendar Cita</h2>
    </x-slot>

    <div class="py-6 max-w-2xl mx-auto px-4">
        <form action="{{ route('secretaria.appointments.store') }}" method="POST"
                ...
                <select name="id_user"
                    ...
                    @foreach($doctors as $doctor)
                        <option value="{{ $doctor->id_user }}"
                            {{ old('id_user') == $doctor->id_user ? 'selected' : '' }}>
                            {{ $doctor->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="notes" rows="3" maxlength="150"
                    class="mt-1 w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit"
                    class="px-4 py-2 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">Agendar</button>
            </div>
        </form>
    </div>
</x-app-layout>