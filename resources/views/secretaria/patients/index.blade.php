<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 ls:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
            {{-- Busqueda --}}
            <form
                action="{{ request()->routeIs('doctor.*') ? route('doctor.patients.index') : route('secretaria.patients.index') }}"
                method="GET" class="flex gap-2 w-full sm:w-auto">
                <input type="text" name="search" value="{{ $search }}" placeholder="Buscar por nombre o DUI..."
                    class="border rounded px-3 py-2 text-sm w-full sm:w-72 focus:outline-none focus:ring">
                <button type="submit"
                    class="bg-gray-700 text-white px-4 py-2 text-sm rounded hover:bg-gray-800">Buscar</button>
                @if ($search)
                    <a href="{{ request()->routeIs('doctor.*') ? route('doctor.patients.index') : route('secretaria.patients.index') }}"
                        class="px-4 py-2 text-sm border rounded hover:bg-gray-500 text-white">Limpiar</a>
                @endif
            </form>
                        ...
                        <tr>
                            ...
                            @if(auth()->user()->role->rol === 'Secretaria')
                                <td class="px-6 py-4 text-sm space-x-2">
                                    {{-- Historial --}}
                                    <a href="{{ route('shared.history.show', $patient->id_patient) }}"
                                        class="text-indigo-600 hover:underline">Historial</a>
