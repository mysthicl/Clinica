        ...
        <form action="{{ route('admin.users.update', $user->id_user) }}" method="POST"
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', $user->email) }}"
                    class="mt-1 w-full border rounded px-3 py-2 text-sm @error('email') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="password" name="password"
                    class="mt-1 w-full border rounded px-3 py-2 text-sm @error('password') border-red-500 @enderror">
            ...
            <div>
                ...
                <select name="id_rol"
                    class="mt-1 w-full border rounded px-3 py-2 text-sm @error('id_rol') border-red-500 @enderror">
                    @foreach($roles as $role)
                        <option value="{{ $role->id_rol }}"
                            {{ old('id_rol', $user->id_rol) == $role->id_rol ? 'selected' : '' }}>
                            {{ $role->rol }}
                        </option>
                    @endforeach
                </select>
